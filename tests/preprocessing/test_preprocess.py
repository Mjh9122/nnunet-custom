...
def setup_dataset():
    ...
    precrop_dims = [[64, 32, 32], [64, 32, 32], [32, 16, 16], [32, 16, 16], [16, 16, 16]]
    postcrop_dims = [[32, 32, 32], [16, 16, 16], [16, 16, 16], [16, 16, 16], [8, 8, 8]]
    spacings = [[.5, .5, .5], [.4, .3, .2], [.4, .4, .4], [1., 1., 1.], [.2, .2, .2]]
    ...
    masks = [np.random.choice([0, 1], size = dims, p = [.2, .8]) for dims in postcrop_dims]

    masked = np.concat([arr[mask != 0].flatten() for arr, mask in zip(arrs, masks)])
    mean = masked.mean()
    std = masked.std()
    low = np.percentile(masked, .5)
    ...
    for i, img in enumerate(imgs):
        sitk.WriteImage(img, TEST_DATA_DIR / 'crops' / 'imagesTr' / f"test0{i + 1}.nii.gz")

    imgs = [sitk.GetImageFromArray(mask) for mask in masks]

    for i, img in enumerate(imgs):
        sitk.WriteImage(img, TEST_DATA_DIR / 'crops' / 'labelsTr' / f"test0{i + 1}.nii.gz")
...
def test_reservoir_CT():
    ...
    nums = nums.reshape((100, 100, 100, 100))
    ...
    for i, arr in enumerate(nums):
        ...
        stats = {
            "pre_crop_shape": np.array((100, 100, 100)),
            "post_crop_shape": np.array((100, 100, 100)),
